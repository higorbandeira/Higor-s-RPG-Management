...
from app.schemas.auth import LoginIn, LoginOut, RefreshOut, MeOut
from app.db.models.user import User
from app.db.models.refresh_token import RefreshToken
from app.core.security import (
    verify_password, create_access_token,
    create_refresh_token_raw, hash_refresh_token, normalize_nickname
)
...
def login(data: LoginIn, response: Response, db: Session = Depends(get_db)):
    ...
    access = create_access_token(str(user.id), user.role, settings.JWT_SECRET, minutes=settings.ACCESS_MINUTES)
    ...
    return LoginOut(accessToken=access, user={"id": user.id, "nickname": user.nickname, "role": user.role})
...
def refresh(request: Request, db: Session = Depends(get_db)):
    ...
    rt = db.query(RefreshToken).filter(RefreshToken.token_hash == token_hash, RefreshToken.revoked_at.is_(None)).first()
