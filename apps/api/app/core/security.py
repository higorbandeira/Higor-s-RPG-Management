...
def create_access_token(subject: str, role: str, secret: str, minutes: int = 15) -> str:
...
def create_refresh_token_raw() -> str:
    # simples (você pode trocar depois por token mais robusto)
    return hashlib.sha256(f"{datetime.now(timezone.utc).timestamp()}".encode()).hexdigest()
